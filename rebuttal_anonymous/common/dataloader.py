...
def create_few_shot_dataset(dataset_name, shots=5, seed=42, device="cuda:0", path_prefix="."):
    ...
    if dataset_name == "arxiv":
        # Cache OGB official splits locally to avoid repeated downloads
        # and PyTorch 2.6+ weights_only incompatibility with OGB
        split_cache = os.path.join(path_prefix, "datasets", "arxiv_ogb_split.pt")
        if os.path.exists(split_cache):
            cached = torch.load(split_cache, weights_only=False)
            split_idx = cached
        else:
            try:
                # Monkey-patch torch.load for OGB compatibility with PyTorch 2.6+
                ...
                torch.load = _orig_load
                raise RuntimeError(
                    f"Failed to load OGB arxiv splits: {e}\n"
                    f"Try: python -c \"\n"
                    f"import torch; torch.load = lambda f, *a, **kw: torch.__orig_load(f, *a, **{{**kw, 'weights_only': False}})\n"
                    f"from ogb.nodeproppred import PygNodePropPredDataset\n"
                    f"d = PygNodePropPredDataset(name='ogbn-arxiv')\n"
                    f"s = d.get_idx_split()\n"
                    f"torch.save(s, '{split_cache}')\n"
                    f"\""
                ) from e
            # Cache for next time
            os.makedirs(os.path.dirname(split_cache), exist_ok=True)
            torch.save(split_idx, split_cache)
            print(f"Cached OGB arxiv splits to {split_cache}")
