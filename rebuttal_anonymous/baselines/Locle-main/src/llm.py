...
def my_llm_answer(topk_prompt, dataname, exp, temperature = 1.2, data = None, strategy = 'random', seed = 0, key_name = 'few_shot_all'):
    n = 3
...
class Experiment:
    ...
    def help_cluster(self, prompts, select_ids, temperature = None, top_p = None, n = 1, dataset = 'cora', key = 'zero_shot', rewrite = False, fix = False, seed = 0, sp = 2, ss = 2):
        responses = []
        prompts[~select_ids] = ""
        cache = self.load_cache(dataset, key)
        fixed_grammar = 0
        ...
        if cache != None:
            cache = cache.get(key, None)
        input_filename = "data/cache/prompt_async_input_{}_cluster.json".format(dataset)
        output_filename = "data/cache/prompt_async_input_{}_cluster.json".format(dataset)
        cache_hit = 0
    ...
    def async_api(self, prompts, label_names, temperature = None, top_p = None, n = 1, dataset = 'cora', key = 'zero_shot', rewrite = False, fix = False, seed = 0, sp = 2, ss = 2):
        ...
        if result_cache is None:
            result_cache_pred = torch.ones(len(prompts)).long() * -1
        else:
            result_cache_pred = result_cache['pred']
        
        valid_number = len([x for x in prompts if x != ""])
        input_filename = "data/cache/prompt_async_input_{}_{}_temperature_{}_n_{}_input_seed_{}_{}.json".format(dataset, key, temperature, n, seed, valid_number)
        output_filename = "data/cache/prompt_async_input_{}_{}_temperature_{}_n_{}_output_seed_{}_{}.json".format(dataset, key, temperature, n, seed, valid_number)
        ...
        openai_result = efficient_openai_text_api(prompts, input_filename, output_filename, sp, ss, api_key=self.api_key, n = n, rewrite = rewrite)
        for i, res in tqdm(enumerate(openai_result)):
            if i not in select_ids:
                responses.append("")
                continue
...
Old one: {} \n New one here:".format(old)
    ...
    def eval_and_predict(self, res, label_names, select_ids, gt, dataset_name = 'cora', method='zero_shot', strategy = 'random', data_obj = None, seed = 0, g_error_idx = None):
        ## accuracy
        ## ECE 
        ## plot
        ## save the result
        ...
        print("all acc: {:.2f}, filter acc: {:.2f}, ece: {:.2f}, number of labels: {}".format(all_acc, filter_acc, ece, len(filter_label)))
        return all_acc, filter_acc, ece, pred
...
def n_hop_neighbor_prompt(data, texts, label_names, need_tasks = True, object_cat = "Paper", question = "Which arxiv cs subcategories does this paper belong to?", \
                    answer_format = "Give 3 likely arXiv CS sub-categories as a comma-separated list ordered from most to least likely together with a confidence ranging from 0 to 100, in the form of a list python dicts like [{\"answer:\":<answer_here>, \"confidence\": <confidence_here>}]"):
    prompts = []
    for idx, text in enumerate(texts):
        ...
        neighbors = k_hop_subgraph(i, 1, data.edge_index, relabel_nodes = False)
    pass

def update_prompt(texts, label_names, need_tasks = True, object_cat = "Paper", question = "Which arxiv cs subcategories does this paper belong to?", description='', reasoning='', data=None, \
                    answer_format = "Give 3 likely arXiv CS sub-categories as a comma-separated list ordered from most to least likely together with a confidence ranging from 0 to 100, in the form of a list python dicts like [{\"answer:\":<answer_here>, \"confidence\": <confidence_here>}]"):
    " From 'Distilling Large Language Models for Text-Attributed Graph Learning' "
    prompts = []
    for i, text in enumerate(texts):
        
        from torch_geometric.utils import k_hop_subgraph   
        subset, _, _, _ = k_hop_subgraph(i, 1, data.edge_index, relabel_nodes = False)
        cosine_similarities = torch.nn.functional.cosine_similarity(data.x[i], data.x[subset])[1:]
        rank_indices = torch.argsort(cosine_similarities, descending=True)
        top2 = rank_indices[:2]
        title = text.split(". :")[0]
        
        prompt = f"Target Paper Title: {title}\n"
        prompt += f"It has following neighbor papers at hop 1:\n"
        # prompt += "It has following important neighbors, from most related to least related:\n"
        
        for (idx, num) in enumerate(rank_indices):
            if idx==0:
                continue
            title = texts[subset[rank_indices[num]].item()].split(". :")[0]
            prompt+= f"Paper {idx+1} title: {title}\n"
        prompt += reasoning + "\n"
        if not 'arxiv' in question:
            prompt += "There are following categories: \n"
            prompt += "[" + ", ".join(label_names) + "]" + "\n"
        ...
        prompts.append(prompt)
    return prompts
def update_prompt2(texts, label_names, need_tasks = True, object_cat = "Paper", question = "Which arxiv cs subcategories does this paper belong to?", description='', reasoning='', data=None, \
                    answer_format = "Give 3 likely arXiv CS sub-categories as a comma-separated list ordered from most to least likely together with a confidence ranging from 0 to 100, in the form of a list python dicts like [{\"answer:\":<answer_here>, \"confidence\": <confidence_here>}]"):
    " From 'Similarity-based Neighbor Selection for Graph LLMs' "
    prompts = []
    for i, text in enumerate(texts):
        
        from torch_geometric.utils import k_hop_subgraph   
        subset, _, _, _ = k_hop_subgraph(i, 1, data.edge_index, relabel_nodes = False)
        cosine_similarities = torch.nn.functional.cosine_similarity(data.x[i].unsqueeze(0), data.x[subset].unsqueeze(0))
        rank_indices = torch.argsort(cosine_similarities[0], descending=True)
        top2 = rank_indices[:2]
        prompt = "{}: \n".format(object_cat)
        prompt += (text + "\n")
        prompt += "It has following important neighbors which has citation relationship to this paper, from most related to least related:\n"
        for i, idx in enumerate(top2):
            prompt += f"Neighbor {i}: {texts[idx]}\n"
        prompt += description + "\n"
        prompt += reasoning + "\n"
        prompt += question
        if need_tasks:
...
def no_category_prompt(texts, label_names, need_tasks = True, object_cat = "Paper", question = "Which arxiv cs subcategories does this paper belong to?", \
                    answer_format = "Give 3 likely arXiv CS sub-categories as a comma-separated list ordered from most to least likely together with a confidence ranging from 0 to 100, in the form of a list python dicts like [{\"answer:\":<answer_here>, \"confidence\": <confidence_here>}]"):
    prompts = []
    for text in texts:
        prompt = "{}: \n".format(object_cat)
...
def calculate_cost_from_a_list_of_texts(texts, select_ids = None):
    ...
    for i, text in enumerate(texts):
        ...
        if len(text) > 0:
            cost += num_tokens_from_string(text, model = 'gpt-3.5-turbo')
    return cost
...
if __name__ == '__main__':
    ...
    for dataname in need_datasets:
        ...
        data = get_dataset(seeds, dataname, args.split, args.data_format, data_path, None, args.pl_noise, args.no_val, args.budget, args.strategy, args.num_centers, args.compensation, args.save_data, args.filter_strategy, args.max_part, args.oracle, reliability_list, args.total_budget, args.second_filter, False, False, args.filter_all_wrong_labels, args.alpha, args.beta, args.gamma, args.ratio, all_args=args)
        print(torch.where(data.train_masks[0])[0])
        
        # print(torch.where(data.train_masks[1])[0].shape)
