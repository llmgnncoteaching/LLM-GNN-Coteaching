...
def kmeans_check(b, x_embed, n_cluster, train_mask, seed, data, device):
    ...
    # check the selected indices and select fewer indices with their neighbors
...
def featprop_query(b, x, edge_index, train_mask, seed, device):
    ...
    selected = torch.tensor(np.array(km.medoids, dtype = np.int64))
...
def greedy_query(budget, x_embed, train_mask, seed, data, device, args):
    graph = copy.deepcopy(data)
    from torch_geometric.utils import to_networkx
    graph = to_networkx(graph)
    features = data.x.cpu().numpy()
    for v in graph.nodes():
        graph.nodes[v]['embedding'] = features[v]
        graph.nodes[v]['SIM'] = 0
    
    algo = greedy(G=graph, B=budget, t=args.greedy_t, k=args.greedy_k)
    
    trainset = torch.where(train_mask)[0].detach().cpu().numpy()
    s = algo.greedyET(trainset)
    return s



class greedy:
    def __init__(self, G=None, dataset='Cora', B=140, sim_metric='COSINE', t=0.9999, k=1, sample_size=0):
    ...
    def SIM(self, embedding1, embedding2):
        ...
        if (self.sim_metric == 'ED'):
            norm_distance = (norm(embedding1-embedding2)-self.minDistance)/self.disRange
            return 1/(1+norm_distance)


    # compute G.nodes[v]['delta']=H_k, G.nodes[v]['H_alphak'], G.nodes[v]['DU'] and G.nodes[v]['nf'].
    def preprocess(self):
        G = self.G
        for v in G.nodes():
            G.nodes[v]['H_alphak'] = G.neighbors(v)
            ...
            if (self.k > 1):
                ...
                while (queue and hop <= self.k):
                    u = queue.pop(0)
                    marking = -1
                    # subset, _, _, _ = k_hop_subgraph(u, 1, G.edge_index)
                    for w in G.neighbors(u):
                        if (w not in visited):
                            ...
                            if (self.sim_metric == 'ED'):
                                # distance = norm(G.x[v].detach().cpu().numpy() - G.x[w].detach().cpu().numpy())
                                distance = norm(G.nodes[v]['embedding'] - G.nodes[w]['embedding'])
                                maxDistance = max(self.maxDistance, distance)
                                minDistance = min(distance, self.minDistance)
                ...
                G.nodes[v]['delta'] = set(G.neighbors(v))

        if (self.sample_size > 0):
            for v in G.nodes():
                G.nodes[v]['H_alphak'] = random.sample(G.nodes[v]['H_alphak'], min(self.sample_size, len(G.nodes[v]['H_alphak'])))

        if (self.sim_metric == 'ED'):
            disRange = self.maxDistance - self.minDistance
        ...
        for v in G.nodes():
            ...
            for w in G.nodes[v]['H_alphak']:
                small, large = min(w, v), max(w, v)
                if ((small, large) not in self.PSIM):
                    self.PSIM[(small, large)] = self.SIM(G.nodes[v]['embedding'], G.nodes[w]['embedding'])
                distances.append(self.PSIM[(small, large)])
            ...
            #print ((quartersum/limit),totalsum/len(distances))        
            G.nodes[v]['DU']=(quartersum/limit)/(totalsum/len(distances))
            
            # nf        
            e=2.71828
            eta=-math.log(2/(self.t+1)-1,e)*G.nodes[v]['DU']
            G.nodes[v]['nf']=eta/totalsum
            #print ("quartersum:",quartersum," totalsum:",totalsum," length:",len(distances))
            #print ("DU:",G.nodes[v]['DU'], " eta:",eta," nf:",G.nodes[v]['nf']," equation:",2/(1+math.exp(-eta/G.nodes[v]['DU']))-1-t)
            #-math.log(2/(t+1)-1,e)/totalsum
            #2 / (1 + math.exp(simscore*math.log(2/(t+1)-1,e)/totalsum)) - 1


    # compute the representative score
    def Rscore(self, G, v, simscore):
        return 2/(1+math.exp(-simscore/G.nodes[v]['DU']*G.nodes[v]['nf']))-1


    def computeGain(self, G, v, S, delta_S, sim_metric):
        I = set()
        triangle_R = 0
        triangle_delta = set()
        tempSIM = {}
        for u in G.nodes[v]['delta']:
            if (u not in delta_S):
                triangle_delta.add(u)

        for r in triangle_delta:
            for u in G.nodes[r]['H_alphak']:
                if (u in delta_S or u in G.nodes[v]['delta']):
                    continue

                if (u not in I):
                    I.add(u)
                    tempSIM[u] = G.nodes[u]['SIM']
                small, large = min(u, r), max(u, r)
                tempSIM[u] += self.PSIM[(small,large)]
        for u in I:
            R1 = self.Rscore(G, u, tempSIM[u])
            triangle_R += self.Rscore(G, u, tempSIM[u]) - self.Rscore(G, u, G.nodes[u]['SIM'])

        return len(triangle_delta) + triangle_R


    # greedy with early termination
    def greedyET(self, training_set):
        G = self.G
        S = list()
        delta_S = set()
        PQ = []
        ...
        while(len(S) < self.B):
            ...
            if (len(S) == 0):
                ...
                for v in training_set:
                    ub = self.computeGain(G, v, S, delta_S, self.sim_metric)
            ...
            # update G.nodes[u]['SIM'] based on bestnode, since S will become S.add(bestnode).
            I = set()
            triangle_delta = set()
            for u in G.nodes[bestnode]['delta']:
                if (u not in delta_S):
                    triangle_delta.add(u)

            for r in triangle_delta:
                for u in G.nodes[r]['H_alphak']:
                    if (u in delta_S or u in G.nodes[bestnode]['delta']):
                        continue
                    # G.nodes[u]['SIM'] += SIM(G.nodes[u]['embedding'], G.nodes[r]['embedding'], sim_metric)
                    small, large = min(u, r), max(u, r)            
                    G.nodes[u]['SIM'] += self.PSIM[(small, large)]
    ...
    def save_seeds(self, S, dataset, sim_metric, B, t, k, ss):
...
def subspace_query(budget, x_embed, train_mask, seed, data, args,  t=2, alpha=1.5, norm_x = False, c= 2**-.5):
    '''
    ...
    for i in range(t):
      ...
      z = alpha**(i+1) * x_embd
      sum_z += z
    subspace = Subspace(sum_z, c, k=budget, class_num=(max(data.y)+1).item(),svd_dim=args.svd_dim,kmeans_dim = args.kmeans_dim, method=args.method)
    indices = subspace.run_model(data=data)
    print("Subspace query done")
    print("Indices: ", indices)
    return indices

class Subspace:
    def __init__(self, H, c, k,class_num, svd_dim,kmeans_dim, method='leiden'):
        self.H = H
        self.c = c
        self.k = k
        if svd_dim == 'c':
            self.svd_dim = class_num
        elif svd_dim == 'k':
            self.svd_dim = k
        else:
            self.svd_dim = int(svd_dim)
        if kmeans_dim == 'c':
            self.kmeans_dim = class_num
        elif kmeans_dim == 'c+1':
            self.kmeans_dim = class_num+1
        elif kmeans_dim == 'k':
            self.kmeans_dim = k
        else:
            self.kmeans_dim = int(float(kmeans_dim)*class_num)
        self.class_num = class_num
        print(class_num)
        self.method = method

    def kmeans_check(self, b, x_embed, n_cluster, seed, data):
        
        seed_everything(seed)
        num_nodes = x_embed.shape[0]
        # ()
        cache_path = osp.join(GLOBAL_RESULT_PATH, 'density_x_{}_{}.pt'.format(num_nodes, n_cluster))
        kmeans = Cluster(n_clusters=n_cluster, n_dim=x_embed.shape[1], seed=seed, device=x_embed.device)
        kmeans.train(x_embed)
        ...
        if hasattr(data, 'drop_idx'):
            density[data.drop_idx] = 0
        return indices, density
    def pagerank_query(self, data, seed):
        seed_everything(seed)
        edges = [(int(i), int(j)) for i, j in zip(data.edge_index[0], data.edge_index[1])]
        nodes = list(range(data.x.shape[0]))
        data.g = nx.Graph()
        ...
        page = torch.tensor(list(pagerank(data.g).values()))
        # normalize to 0 to 1
        page = (page - page.min()) / (page.max() - page.min())
        # page[np.where(train_mask == 0)[0]] = 0
        return page

    def run_model(self, pagerank=None, density2=None,alpha=1, beta=0., gamma=0., data=None):
        from sklearn.preprocessing import StandardScaler
        from sklearn.decomposition import TruncatedSVD
        from sklearn.cluster import KMeans
        # ()
        # H = StandardScaler(with_std=False).fit_transform(self.H)
        # ()
        svd = TruncatedSVD(self.svd_dim)
        svd.fit(self.H.T)
        U = svd.components_.T
        Q = U
        Q = torch.tensor(Q, dtype=torch.float32)
        kmeans = Cluster(n_clusters=self.kmeans_dim, n_dim=Q.shape[1],seed=42,device='cpu')
        kmeans.train(Q)

        # Calculate density
        centers = kmeans.get_centroids()
        label = kmeans.predict(Q)
        centers = centers[label]
        dist_map = torch.linalg.norm(Q - centers, dim=1)
        dist_map = torch.tensor(dist_map, dtype=Q.dtype, device=Q.device)
        density = 1 / (1 + dist_map)
        
        _, indices = torch.topk(alpha*density ,k=self.k)
        return indices

    def square_feat_map(self, z, c=1):
