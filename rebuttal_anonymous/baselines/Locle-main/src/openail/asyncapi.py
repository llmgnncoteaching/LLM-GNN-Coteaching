...
async def process_api_requests_from_file(
    ...
    max_attempts: int,
    logging_level: int,
    seconds_to_pause: int = 60, 
    seconds_to_sleep: int = 0.1
):
    """Processes API requests in parallel, throttling to stay under rate limits."""
    # constants
    seconds_to_pause_after_rate_limit_error = seconds_to_pause
    seconds_to_sleep_each_loop = seconds_to_sleep  # 1 ms limits max throughput to 1,000 requests per second
    ...
    if request_url.endswith("completions"):
        ...
        api_endpoint = "chat/completions/?api-version=2023-08-01-preview"
        request_header = {'Content-Type': 'application/json', 'api-key': api_key }
    # request_header = {"Authorization": f"Bearer {api_key}"}
    ...
    with open(requests_filepath) as file:
        ...
        while True:
            ...
            if next_request is None:
                if not queue_of_requests_to_retry.empty():
                    ...
                    try:
                        ...
                        # if 'messages' in request_json and request_json['messages'][0]['content']:
                        #     # content_length = len(request_json['messages'][0]['content'])
                        #     await asyncio.sleep(seconds_to_sleep_each_loop)  # sleep proportionally to content length
            ...
            # if request_json['messages'][0]['content'] != '':
...
class APIRequest:
    ...
    async def call_api(
        ...
        error = None
        if self.request_json['messages'][0]['content'] == '':
            status_tracker.num_tasks_in_progress -= 1
            status_tracker.num_tasks_succeeded += 1
            append_to_jsonl([self.request_json, "", self.task_id], save_filepath)
        else:
            try:
            ...
            if error:
                ...
                if self.attempts_left:
                    ...
                    append_to_jsonl([self.request_json, [str(e) for e in self.result], self.task_id, 'error'], save_filepath)
                    status_tracker.num_tasks_in_progress -= 1
                    status_tracker.num_tasks_failed += 1
            else:
                append_to_jsonl([self.request_json, response, self.task_id], save_filepath)
                status_tracker.num_tasks_in_progress -= 1
                status_tracker.num_tasks_succeeded += 1
...
def num_tokens_consumed_from_request(
    ...
    if api_endpoint.endswith("completions") or api_endpoint.endswith("completions/?api-version=2023-08-01-preview"):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_tokens_per_minute", type=int, default=90000)
