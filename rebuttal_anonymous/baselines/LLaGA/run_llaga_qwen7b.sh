...
OUT=./checkpoints/${D}/llaga-qwen7b-mpnet-nc
R=/home/anon/graph-token-reasoning/outputs/LLAGAQWEN7B_${D}.txt
echo "=== START $(date) LLaGA-Qwen7B D=$D GPU=$G ===" > $R
CUDA_VISIBLE_DEVICES=$G $PY train/train_mem_qwen.py \
  --model_name_or_path Qwen/Qwen2.5-7B-Instruct --version mpt --cache_dir ../../checkpoint \
  ...
  --output_dir $OUT --num_train_epochs 3 --per_device_train_batch_size 4 \
  --gradient_accumulation_steps 4 --max_grad_norm 1.0 --save_strategy no --learning_rate 5e-4 \
