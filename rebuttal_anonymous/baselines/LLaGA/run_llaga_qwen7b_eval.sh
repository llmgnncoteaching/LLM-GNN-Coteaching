...
CK=./checkpoints/${D}/llaga-qwen7b-mpnet-nc
ANS=/home/anon/graph-token-reasoning/outputs/llagaqwen7b_${D}_ans.jsonl; rm -f $ANS
R=/home/anon/graph-token-reasoning/outputs/LLAGAQWEN7B_${D}.txt
echo "=== EVAL START $(date) D=$D ===" >> $R
CUDA_VISIBLE_DEVICES=$G $PY eval/eval_pretrain_qwen.py --model_path $CK --model_base Qwen/Qwen2.5-7B-Instruct --conv_mode mpt --dataset $D --pretrained_embedding_type mpnet --use_hop 2 --sample_neighbor_size 10 --answers_file $ANS --task nc --cache_dir ../../checkpoint --template ND >> $R 2>&1
$PY - "$ANS" "$D" >> $R 2>&1 <<'PYS'
import json,sys,re
