        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            if 'opt' in model_base:
                ...
                model = LlagaOPTForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=cfg_pretrained,
                                                              cache_dir=cache_dir,
                                                              **kwargs)
            elif 'qwen' in model_base.lower():  # jun18_llaga_qwen
                tokenizer = AutoTokenizer.from_pretrained(model_base)
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
                model = LlagaQwenForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=cfg_pretrained, cache_dir=cache_dir, **kwargs)
            else:
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
            ...
            if 'qwen' in model_path.lower():  # jun18_llaga_qwen full-model load
                tokenizer = AutoTokenizer.from_pretrained(model_path)
                model = LlagaQwenForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, **kwargs)
            else:
                tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False)
