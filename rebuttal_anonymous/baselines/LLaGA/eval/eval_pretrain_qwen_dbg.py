...
def eval_model(args):
    ...
    if os.path.exists(f"dataset/{args.dataset}/processed_data.pt"):
        ...
        try:
            with torch.inference_mode():
                _, _, _, inputs_embeds, _ = model.prepare_inputs_labels_for_multimodal(
                    input_ids, None, None, None, graph.cuda(), graph_emb.half().cuda())
                cur = inputs_embeds
                gen_ids = []
                for _step in range(12):  # jun18_llaga_qwen manual greedy decode (bypass HF generate)
                    mout = model.model(inputs_embeds=cur)
                    nlogits = model.lm_head(mout.last_hidden_state[:, -1, :])
                    nxt = nlogits.argmax(dim=-1)
                    tid = int(nxt.item())
                    if tid == tokenizer.eos_token_id:
                        break
                    gen_ids.append(tid)
                    e = model.get_model().embed_tokens(nxt).unsqueeze(1)
                    cur = torch.cat([cur, e], dim=1)
            outputs = tokenizer.decode(gen_ids, skip_special_tokens=True).strip()
