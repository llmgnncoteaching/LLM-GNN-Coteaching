...
def preprocess_llama_2(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        if cur_len < tokenizer.model_max_length:
            if abs(cur_len - total_len) > 8:  # jun18_llaga_qwen: tolerate small ChatML offset (single-turn answer mask is correct)
                target[:] = IGNORE_INDEX
                print(
...
def preprocess_mpt(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        target[cur_len:] = IGNORE_INDEX
        if cur_len < tokenizer.model_max_length:
            if abs(cur_len - total_len) > 8:  # jun18_llaga_qwen relaxed guard (answer mask is correct)
                target[:] = IGNORE_INDEX
...
class LazySupervisedGraphDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.use_dataset = [data_args.use_dataset] if os.path.exists(f"dataset/{data_args.use_dataset}/processed_data.pt") else data_args.use_dataset.split('-')
        ...
        for d, dataset in enumerate(self.use_dataset):
            ...
            if "." in dataset:
                ...
                repeat=int(ds[1])
                dataset=ds[0]
            if os.path.exists(f"dataset/{dataset}/processed_data.pt"):
                data_path = f"dataset/{dataset}/processed_data.pt"
...
def _train():
    ...
    elif data_args.pretrained_embedding_type in ["mpnet"]:
    ...
    elif 'qwen' in model_args.model_name_or_path.lower():  # jun18_llaga_qwen
    ...
    if model_args.version == "v0":
        ...
        tokenizer.pad_token = tokenizer.unk_token if tokenizer.unk_token is not None else tokenizer.eos_token  # jun18_llaga_qwen
    else:
        tokenizer.pad_token = tokenizer.unk_token if tokenizer.unk_token is not None else tokenizer.eos_token  # jun18_llaga_qwen
