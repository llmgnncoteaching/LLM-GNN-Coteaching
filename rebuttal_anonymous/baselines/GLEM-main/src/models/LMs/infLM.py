...
class LmInfTrainer:
    ...
    def __init__(self, cf):
        ...
        _is_llama = 'llama' in str(self.cf.hf_model).lower()
        _load_kw = {'attn_implementation': 'eager', 'torch_dtype': th.bfloat16} if _is_llama else {}
        bert_model = AutoModel.from_pretrained(self.cf.hf_model, **_load_kw)
        if _is_llama:
            ...
            bert_model = get_peft_model(bert_model, lora_cfg)
        ...
        # Llama ckpt holds only the trainable delta (LoRA + head); base is reloaded from pretrained.
        self.model.load_state_dict(th.load(ckpt := self.cf.lm.ckpt, map_location='cpu'), strict=not _is_llama)
    ...
    def inference_pred_and_emb(self):
        ...
        inference_args = TrainingArguments(
            ...
            fp16_full_eval=not _is_llama,
            bf16_full_eval=_is_llama,
            remove_unused_columns=False,  # forward(**input) has no named args; keep all cols
        )
