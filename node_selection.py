...
def load_llm_predictions_for_selected(llm_predictions_file: str, selected_node_ids: list, graph_data) -> Dict[int, int]:
    """Returns {node_id: label_id}. Also stores logprobs in a separate dict."""
    label_map = {label.lower().strip(): i for i, label in enumerate(graph_data.label_name)}
...
def load_llm_logprobs_for_selected(llm_predictions_file: str, selected_node_ids: list) -> Dict[int, float]:
    """Returns {node_id: disc_logprob} for calibrated confidence-based selection.
    disc_logprob = logprob of the discriminative token (e.g. 'lg' in 'arxiv cs lg').
    Falls back to avg_logprob if disc_logprob not available."""
    ...
    for i in range(n):
        node_id = int(selected_node_ids[i])
        rec = raw[i]
        # Prefer discriminative logprob (calibrated), fall back to avg
        lp = rec.get('disc_logprob', rec.get('avg_logprob', None))
        if lp is not None:
            logprobs[node_id] = float(lp)
